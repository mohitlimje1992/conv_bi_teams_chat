...
"""
    ...
    {
        "input": "What is the Customer Average Order Value across all the customers",
        "query": """
-- Calculate the overall average order value across all customers.
...
SELECT
  ROUND(AVG(TotalSales * 1.0 / NULLIF(OrderCount,0)), 2) AS [Customer Average Order Value]
FROM CustomerStats;
"""
    },
    {
        ...
        "query": """
-- Find top-5 customers with AvgOrderValue > global average.
WITH CustomerStats AS (
  SELECT
    customer_id,
    customer_name,
...
SELECT TOP 5
  ca.customer_id,
  ca.customer_name,
  ROUND(ca.AvgOrderValue, 2) AS [Avg Order Value]
FROM CustomerAvg ca
CROSS JOIN OverallAvg oa
WHERE ca.AvgOrderValue > oa.GlobalAvgValue
ORDER BY ca.AvgOrderValue DESC;
"""
    },
    {
        "input": "What are the top 3 product sub categories these top 5 high-value customers are buying the most?",
        "query": """
-- Top 3 sub_categories by total sales for the top-5 high-value customers.
WITH CustomerStats AS (
  SELECT
    customer_id,
    SUM(sales)              AS TotalSales,
...
JOIN dbo.product       AS p ON o.product_id = p.product_id
WHERE o.customer_id IN (SELECT customer_id FROM TopCustomers)
GROUP BY p.sub_category
ORDER BY TotalSales DESC;
"""
    },
    {
        "input": "Also add category and average order value columns to this data",
        "query": """
-- Extend the previous to include category and each sub_category’s AvgOrderValue.
WITH CustomerStats AS (
  SELECT
    customer_id,
    SUM(sales)              AS TotalSales,
...
SELECT TOP 3
  ...
  p.sub_category,
  SUM(o.sales)                    AS TotalSales,
  ROUND(AVG(ca.AvgOrderValue),2)  AS [Avg Order Value]
FROM dbo.orders_data AS o
JOIN dbo.product       AS p  ON o.product_id = p.product_id
JOIN CustomerAvg       AS ca ON o.customer_id = ca.customer_id
WHERE o.customer_id IN (SELECT customer_id FROM TopCustomers)
...
"""
    },
    {
        ...
        "query": """
-- CLV as total profit for the top-5 high-value customers.
WITH CustomerStats AS (
  SELECT
    customer_id,
    SUM(sales)              AS TotalSales,
...
SELECT
  ...
  ROUND(SUM(o.profit), 2) AS CustomerLifetimeValue
FROM dbo.orders_data AS o
WHERE o.customer_id IN (SELECT customer_id FROM TopCustomers)
...
"""
    ...
    {
        "input": "Compare the average profit margin per segment per region",
        "query": """
-- Avg profit margin (%) = SUM(profit)/SUM(sales)*100, by segment & region.
SELECT
  segment          AS [Customer Segment],
  region,
  CASE
    WHEN SUM(sales) = 0 THEN 0
    ELSE ROUND((SUM(profit)*1.0 / SUM(sales)) * 100, 2)
...
"""
    ...
    {
        "input": "Break the profit margin in Consumer segment and central region further by category",
        "query": """
-- Profit margin by category for Consumer segment in the Central region.
SELECT
  p.category,
  CASE
    WHEN SUM(o.sales) = 0 THEN 0
    ELSE ROUND((SUM(o.profit)*1.0 / SUM(o.sales)) * 100, 2)
